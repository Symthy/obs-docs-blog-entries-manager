...
@pytest.fixture
def blog_to_doc_entry_mapping():
    data_path = resolve_test_data_dir_path()
    return BlogToDocEntryMapping(file_system.join_path(data_path, 'blog_to_doc_mapping.json'))


def test_find_doc_entry_id(blog_to_doc_entry_mapping):
    doc_entry_id = blog_to_doc_entry_mapping.find_doc_entry_id(BlogEntryId('13574176438059028795'))
    assert_that(doc_entry_id.value).is_equal_to('20220201211144')


def test_find_blog_entry_id(blog_to_doc_entry_mapping):
    blog_entry_id = blog_to_doc_entry_mapping.find_blog_entry_id(DocEntryId('20220201211144'))
    assert_that(blog_entry_id.value).is_equal_to('13574176438059028795')


def test_find_blog_entry_ids(blog_to_doc_entry_mapping):
    ...
    blog_entry_ids = blog_to_doc_entry_mapping.find_blog_entry_ids(doc_ids)

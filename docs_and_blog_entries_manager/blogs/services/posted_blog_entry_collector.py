...
class PostedBlogEntryCollector:
    ...
    def execute(self) -> List[PostedBlogEntry]:
        ...
        for posted_blog_entry in self.__blog_entry_repository.all():
            photo_entry_ids: List[str] = md_file.extract_photo_entry_id(self.__hatena_id, posted_blog_entry.content)
    ...
    def __insert_photo_entries(self, posted_blog_entry: PostedBlogEntry, photo_entry_ids: List[str]) -> PostedBlogEntry:
